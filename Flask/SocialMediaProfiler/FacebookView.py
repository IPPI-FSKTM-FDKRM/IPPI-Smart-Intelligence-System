...
@app.route("/facebook/profile/relation/<id>")
@login_required
def facebookRelation(id):
    print "relation"
    cache_comment, cache_like = fb.get_relation_post(id)
    relationProfile = fb.getProfileInstance(id)
    currentUserId = fb.getUsername()
    name = fb.getProfile(currentUserId)['name']
    relationLocation = fb.getLocationRelatioin()

    currentUserId = "1362815530452737"
