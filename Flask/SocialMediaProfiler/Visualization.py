...
def lineChart(dict,stringX,stringY):
    ...
    pltpie.plot(x,y,'w-')
    pltpie.xticks(x)
    pltpie.xlabel(stringX,{'color':'white'})
    pltpie.ylabel(stringY,{'color':'white'})
    ax = pltpie.subplot()
    for i in ax.spines:
...
def barChart(dict,stringX,stringY):
    ...
    for a in yv:
        y.append(len(a))
    pltbar.ylim((min(y)+0.5),(max(y)+0.5))
    bar = pltbar.bar(y_plot, y, align='center')
    for i in range(0,len(bar)):
       bar[i].set_color("white")
    pltbar.xticks(y_plot, x)
    ax = pltbar.subplot()
    pltbar.ylabel(stringY,{'color':'white'})
    pltbar.xlabel(stringX,{'color':'white'})
    for i in ax.spines:
        ax.spines[i].set_color('white')
    ax.xaxis.label.set_color('white')
    ax.yaxis.label.set_color('white')
